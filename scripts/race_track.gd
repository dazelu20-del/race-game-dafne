...
const TRACK_MODEL_PATH := "res://assets/race-track/low_poly_race_track.glb"

const ROAD_MESH_KEYWORDS: Array[String] = [
	"road", "bridge", "starting", "finish", "crooked", "springboard",
	"track", "plane", "floor", "ground", "asphalt", "path",
]
...
func _add_track_model() -> void:
	var track_scene: PackedScene = load(TRACK_MODEL_PATH)
	if track_scene == null:
		push_error("Failed to load track model: %s" % TRACK_MODEL_PATH)
		return

	var track_model := track_scene.instantiate()
	track_model.name = "GeneratedTrackModel"
	# Scale XZ to match the centerline; keep Y at model height so cars align with the road.
	track_model.scale = Vector3(RaceTrackData.TRACK_SCALE, 1.0, RaceTrackData.TRACK_SCALE)
	add_child(track_model)
	call_deferred("_add_track_collision", track_model)
...
func _add_track_collision(track_root: Node3D) -> void:
	...
	for mesh_instance in track_root.find_children("*", "MeshInstance3D", true, false):
		if not _is_drivable_mesh(mesh_instance):
			continue
	...
	if shape_count == 0:
		push_warning("RaceTrack: no road meshes matched; using all track meshes for collision")
		for mesh_instance in track_root.find_children("*", "MeshInstance3D", true, false):
			if not _is_collision_worthy(mesh_instance as MeshInstance3D, track_root):
				continue
			_add_trimesh_collision(track_root, static_body, mesh_instance as MeshInstance3D)
