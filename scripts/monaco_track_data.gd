...
static func to_vec3(point: Vector2, height: float = 0.0) -> Vector3:
...
static func is_on_track_3d(point: Vector3) -> bool:
	return is_on_track(to_vec2(point))
...
static func get_start_grid() -> Array[Dictionary]:
	...
	grid.append({"position": to_vec3(front_row + right * col_gap), "rotation": rot})
	grid.append({"position": to_vec3(front_row - right * col_gap), "rotation": rot})
	grid.append({"position": to_vec3(back_row + right * col_gap), "rotation": rot})
	grid.append({"position": to_vec3(back_row - right * col_gap), "rotation": rot})
	return grid
...
static func get_turn_label_positions() -> Array[Dictionary]:
	...
	for data in TURN_LABELS:
		...
		labels.append({
			"name": data["name"],
			"position": to_vec3(point + normal * (HALF_WIDTH + 28.0), 3.0),
			"rotation": tangent.angle(),
		})
	return labels
